...
public abstract class BrickObject extends PhysicalObject {
	...
	private void removeDeadBricks() {
		...
		for (Brick brick : toRemove) {
			// register change to bricks for updating the view of this
			// BrickObject
			bricksChange = true;

			// get the indices if the bricks connected to the brick being
			// removed
	...
	public ArrayList<double[]> getLines() {
		...
		for (Brick brick : bricks) {
			...
			for (int i = 0; i < brickLines.size(); i++) {
				double[] line = new double[] { brickLines.get(i)[0],// +
																	// position.getX(),
