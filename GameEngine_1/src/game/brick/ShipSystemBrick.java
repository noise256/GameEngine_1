...
public class ShipSystemBrick extends SystemBrick {
	...
	public ArrayList<Float> getNormals(float radius) {
		if (normals == null) {
			...
			for (int i = 0; i < numSegments; i++) {
				for (int j = 0; j < numSegments; j++) {
					norm = new float[] { bx - e + s * i, by - e + s * j, 1.0f };
					norm = MathBox.normalise(norm);
					normals.add(norm[0]);
					normals.add(norm[1]);
					normals.add(norm[2]);

					norm = new float[] { bx - e + s * i, by - e + s * j + s, 1.0f };
					norm = MathBox.normalise(norm);
					normals.add(norm[0]);
					normals.add(norm[1]);
					normals.add(norm[2]);

					norm = new float[] { bx - e + s * i + s, by - e + s * j, 1.0f };
					norm = MathBox.normalise(norm);
					normals.add(norm[0]);
					normals.add(norm[1]);
					normals.add(norm[2]);
					...
					norm = new float[] { bx - e + s * i + s, by - e + s * j + s, 1.0f };
					norm = MathBox.normalise(norm);
					normals.add(norm[0]);
					normals.add(norm[1]);
					normals.add(norm[2]);
	...
	public ArrayList<double[]> getLines() {
		...
		// left
		double[] l4 = new double[] { x - edgeLength / 2, y - edgeLength / 2, x - edgeLength / 2, y + edgeLength / 2 };
