...
public class MathBox {
	...
	public static double getDistanceBetweenPoints(double x1, double y1,
			double x2, double y2) {
...
//	public static <G extends WorldObject> ArrayList<G> getNeighbours(G entity,
...
//				continue;
//			}
//			double d = entity.getGeometry().getPosition().distance(entities.get(i).getGeometry().getPosition());
//			if (d >= min && d <= max) {
...
//	}
	...
	public static double[] rotatePointZ(double[] o, double[] p, double a) {
		...
		return new double[] { Math.cos(a) * v[0] - Math.sin(a) * v[1] + o[0],
				Math.sin(a) * v[0] + Math.cos(a) * v[1] + o[1] };
	}
	...
	//TODO rewrite these to use only two calls to math.cos and math.sin or look up table???
...
//	public static Vector2D rotateAroundPivot(Vector2D pivot, Vector2D point, double angle) {
//		Vector2D translated = point.subtract(pivot);
//		return new Vector2D(
//				translated.getX() * Math.cos(angle) + translated.getY() * Math.sin(angle),
//				-translated.getX() * Math.sin(angle) + translated.getY() * Math.cos(angle)
//		).add(pivot);
//	}
}
